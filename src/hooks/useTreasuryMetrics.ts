...
export const useMarketCap = (subgraphUrl?: string) => {
  ...
  return useTokenSuppliesQuery(
    ...
    {
      select: data => getOhmCirculatingSupply(data.tokenSupplies) * (ohmPrice.data || 0),
      ...QUERY_OPTIONS,
      enabled: latestDateQuery.isSuccess && ohmPrice.isSuccess, // Only fetch when we've been able to get the latest date and price
    },
  );
};
