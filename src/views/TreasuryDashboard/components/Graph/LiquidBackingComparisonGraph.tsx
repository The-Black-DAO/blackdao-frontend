 ...
 * BLKD price and liquid backing per floating BLKD.
 */
export const LiquidBackingPerOhmComparisonGraph = ({ subgraphUrl, earliestDate, activeToken }: GraphProps) => {
  ...
  useMemo(() => {
    ...
    const itemNames: string[] = isActiveTokenOHM
      ? [t`BLKD Price`, t`Liquid Backing per Floating BLKD`]
      : [t`gBLKD Price`, t`Liquid Backing per gBLKD`];
    ...
    setHeaderText(isActiveTokenOHM ? t`BLKD Backing` : t`gBLKD Backing`);
    setTooltipText(
      isActiveTokenOHM
        ? t`This chart compares the price of BLKD against its liquid backing. When BLKD is above liquid backing, the difference will be highlighted in green. Conversely, when BLKD is below liquid backing, the difference will be highlighted in red.`
        : t`This chart compares the price of gBLKD against its liquid backing. When gBLKD is above liquid backing, the difference will be highlighted in green. Conversely, when gBLKD is below liquid backing, the difference will be highlighted in red.`,
    );
  }, [isActiveTokenOHM]);
