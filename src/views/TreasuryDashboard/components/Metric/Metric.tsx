...
export const MarketCap: React.FC<AbstractedMetricProps & MetricSubgraphProps> = props => {
  ...
  const _props: MetricProps = {
    ...props,
    label: t`BLKD Market Cap`,
    tooltip: t`Market capitalization is the dollar value of the outstanding BLKD tokens. It is calculated here as the price of BLKD multiplied by the circulating supply. 
    
    As the displayed BLKD price is rounded to 2 decimal places, a manual calculation using the displayed values is likely to slightly differ from the reported market cap. The reported market cap is accurate, as it uses the unrounded price of BLKD.
...
/**
 * same as OHMPriceFromSubgraph but uses BLKD-DAI on-chain price
 */
...
export const OhmCirculatingSupply: React.FC<AbstractedMetricProps & MetricSubgraphProps> = props => {
  ...
  const _props: MetricProps = {
    ...props,
    label: t`BLKD Circulating Supply / Total`,
    tooltip: t`Circulating supply is the quantity of outstanding BLKD not owned by the protocol (excluding BLKD in LPs).`,
  };
...
export const GOhmCirculatingSupply: React.FC<AbstractedMetricProps> = props => {
  const _props: MetricProps = {
    ...props,
    label: t`gBLKD Circulating Supply / Total`,
    tooltip: t`gBLKD supply is synthetically derived from BLKD supply divided by the index.`,
  };
...
export const BackingPerOHM: React.FC<AbstractedMetricProps & MetricSubgraphProps> = props => {
  ...
  /**
   * Liquid backing per BLKD floating is used as the metric here.
   * Liquid backing does not include BLKD in protocol-owned liquidity,
   * so it makes sense to do the same for the denominator, and floating supply
   * is circulating supply - BLKD in liquidity.
   */
  ...
  const tooltip = t`Liquid backing is divided by floating supply of BLKD to give liquid backing per BLKD.
  
  Floating supply of BLKD is the quantity of outstanding BLKD not owned by the protocol (including BLKD in LPs): ${
...
export const BackingPerGOHM: React.FC<AbstractedMetricProps & MetricSubgraphProps> = props => {
  ...
  const tooltip = t`Liquid backing per gBLKD is synthetically calculated as liquid backing multiplied by the current index and divided by BLKD floating supply.`;

  const _props: MetricProps = {
    ...props,
    label: t`Liquid Backing per gBLKD`,
    tooltip: tooltip,
  };
...
export const CurrentIndex: React.FC<AbstractedMetricProps & MetricSubgraphProps> = props => {
  ...
  const _props: MetricProps = {
    ...
    tooltip: t`The current index tracks the amount of sBLKD accumulated since the beginning of staking. Basically, how much sBLKD one would have if they staked and held 1 BLKD from launch.`,
  };

  if (currentIndex) _props.metric = `${formatNumber(currentIndex, 2)} sBLKD`;
  else _props.isLoading = true;
