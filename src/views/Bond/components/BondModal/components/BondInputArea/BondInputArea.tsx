...
export const BondInputArea: React.VFC<{
  ...
  ${isInverseBond ? props.bond.baseToken.name : `sBLKD`}`;
            ...
            <TokenAllowanceGuard
              ...
              message={
                <>
                  ...
                  <Trans>Please approve Black DAO to use your</Trans> <b>{props.bond.quoteToken.name}</b>{" "}
          ...
          balance={
            <span>
              ...
              {!isInverseBond && !!currentIndex && (
                <span>
                  (≈{amountInBaseToken.div(currentIndex).toString({ decimals: 4, format: true, trim: false })} gBLKD)
                </span>
              )}
            </span>
          }
          tooltip={t`The total amount of payout asset you will recieve from this bond purchase. (sBLKD quantity will be higher due to rebasing)`}
        />
