  ...
  return (
    <Paper headerText="Your Bonds">
      <Box display="flex" flexDirection="column" alignItems="center">
        ...
        >
          <Tab aria-label="payout-sohm-button" label="sBLKD" style={{ fontSize: "1rem" }} />
          <Tab aria-label="payout-sohm-button" label="gBLKD" style={{ fontSize: "1rem" }} />
        </Tabs>
      </Box>
        ...
        <Box display="flex" flexDirection="column" alignItems="center" mt="24px" width={isSmallScreen ? "100%" : "50%"}>
          ...
          <Box mt="4px" mb="8px">
            <Typography variant="h4" align="center">
              {isPayoutGohm
                ? `${totalClaimableBalance.toString({ decimals: 4, format: true })} gBLKD`
                : `${currentIndex?.mul(totalClaimableBalance).toString({ decimals: 4, format: true })} sBLKD`}
            </Typography>
          </Box>
            ...
            {notes.map(note => (
              <Box mt="32px">
                ...
                <Box display="flex" justifyContent="space-between" mt="16px">
                  ...
                  <Typography>
                    {isPayoutGohm
                      ? `${note.payout.toString({ decimals: 4, format: true })} gBLKD`
                      : `${currentIndex?.mul(note.payout).toString({ decimals: 4, format: true })} sBLKD`}
                  </Typography>
                </Box>
