...
import { OHM_TOKEN, DAI_TOKEN } from "src/constants/tokens";
...
export const fetchBond = async ({ id, isInverseBond, networkId }: UseBondOptions) => {
  ...
  const isSoldOut = isInverseBond
    ...
    console.log('debug bond cap', id, isInverseBond, capacity, capacityInBaseToken, capacityInBaseToken.lt("1"),  isSoldOut)
  return {
    id,
    baseToken,
    quoteToken,
