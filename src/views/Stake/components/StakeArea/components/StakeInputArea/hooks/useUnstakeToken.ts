...
export const useUnstakeToken = (fromToken: "sBLKD" | "gBLKD") => {
  ...
  const addresses = fromToken === "sBLKD" ? SOHM_ADDRESSES : GOHM_ADDRESSES;
  ...
  return useMutation<ContractReceipt, Error, string>(
    async amount => {
      ...
      const _amount = new DecimalBigNumber(amount, fromToken === "gBLKD" ? 18 : 9);
      ...
      const shouldRebase = fromToken === "sBLKD";
    ...
    },
    {
      ...
      onSuccess: async (tx, amount) => {
        trackGAEvent({
          ...
          value: new DecimalBigNumber(amount, fromToken === "gBLKD" ? 18 : 9).toApproxNumber(),
