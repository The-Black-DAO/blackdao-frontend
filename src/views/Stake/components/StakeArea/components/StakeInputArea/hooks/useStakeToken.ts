...
export const useStakeToken = (toToken: "sBLKD" | "gBLKD") => {
  ...
  return useMutation<ContractReceipt, Error, string>(
    async amount => {
      ...
      const shouldRebase = toToken === "sBLKD";
    ...
    },
    {
      ...
      onSuccess: async (tx, amount) => {
        ...
        const keysToRefetch = [
          ...
          balanceQueryKey(address, toToken === "sBLKD" ? SOHM_ADDRESSES : GOHM_ADDRESSES, networks.MAINNET),
        ];
        ...
        dispatch(createInfoToast(t`Successfully staked BLKD`));
      },
    },
  );
};
