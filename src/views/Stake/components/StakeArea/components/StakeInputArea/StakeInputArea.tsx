...
export const StakeInputArea: React.FC<{ isZoomed: boolean }> = props => {
  ...
  const [stakedAssetType, setStakedAssetType] = useState<"sBLKD" | "gBLKD">("sBLKD");
  const [currentAction, setCurrentAction] = useState<"STAKE" | "UNSTAKE">("STAKE");

  const fromToken = currentAction === "STAKE" ? "BLKD" : stakedAssetType;

  // Max balance stuff
  const [amount, setAmount] = useState("");
  const addresses = fromToken === "BLKD" ? OHM_ADDRESSES : fromToken === "sBLKD" ? SOHM_ADDRESSES : GOHM_ADDRESSES;
  ...
  return (
    <StyledBox mb={3}>
      {currentAction === "UNSTAKE" && liveInverseBonds && (
        <InfoNotification>
          {t`Unstaking your BLKD? Trade for Treasury Stables with no slippage & zero trading fees via`}
          &nbsp;
          ...
          message={
            currentAction === "STAKE" ? (
              <>
                ...
                <br />
                <Trans>Please approve Black DAO to use your</Trans> <b>BLKD</b> <Trans>for staking</Trans>.
              </>
            ) : (
              <>
                <Trans>First time unstaking</Trans> <b>{fromToken === "sBLKD" ? "sBLKD" : "gBLKD"}</b>?
                <br />
                <Trans>Please approve Black DAO to use your</Trans> <b>{fromToken === "sBLKD" ? "sBLKD" : "gBLKD"}</b>{" "}
      ...
      <Paper className="ohm-card confirm-dialog">
        <Box display={[null, "flex"]} alignItems="center" justifyContent="space-between">
          <Grid component="label" container alignItems="center" spacing={1} wrap="nowrap">
            ...
            <Grid item>
              <Switch
                ...
                checked={stakedAssetType === "gBLKD"}
                inputProps={{ "aria-label": "stake to gohm" }}
                onChange={(_, checked) => setStakedAssetType(checked ? "gBLKD" : "sBLKD")}
              />
            </Grid>
          ...
          <Box marginTop={[2, 0]} flexShrink={0}>
            {stakedAssetType === "gBLKD" && <GOHMConversion amount={amount} action={currentAction} />}
          </Box>
        </Box>
      </Paper>
    </StyledBox>
